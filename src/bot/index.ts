...
export const dataMsgs: Message[] = [];
export const changeChats = (newChat: Chat) => {
  chats = chats.map((chat) =>
    chat.chatId === newChat.chatId ? newChat : chat
  );
};
...
bot.action("settings", async (ctx) => {
  try {
    ...
    if (ctx.from.username !== process.env.USER_NAME)
      ...
      await ctx.replyWithHTML(
        "<b>Изменение настроек доступно только в групповых чатах!</b>"
      );
    else if (chats.map(({ chatId }) => chatId).indexOf(ctx.chat.id) >= 0)
      await ctx.replyWithHTML(
        "<b>Данный чат подключен</b>",
        Markup.inlineKeyboard([
